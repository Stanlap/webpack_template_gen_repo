function calcCCAndGFR(gender, age, weight, race, creatinVal, crUns) {
    ...
    function getCKD_EPIResGFR(gender, age, race, creatinVal, cU, SKD_EPI) {
        let cV = (creatinVal / uniformCrValue(cU, vC)).toFixed(2);
        if (gender == 0) {
    ...
    let cc = parseInt(((140 - age) * weight) / (72 * creatinVal) * uniformCrValue(crUns, vC = 1) * findGenderCoefCockcroftGault(gender));
